...
async fn async_ad(a: i32, b: i32) -> i32 {
...
fn add_handler(
    ...
    match (a, b, id) {
        (Some(a), Some(b), None) => {
            let handle = TOKIO_RUNTIME.spawn(async_ad(a, b));
        ...
        (None, None, Some(id)) => {
            ...
            let result = match TOKIO_RUNTIME.block_on(handle) {
                Ok(result) => result,
...
pub fn send_add(a: i32, b: i32) -> Result<String, String> {
    match add_handler(Some(a), Some(b), None) {
        ...
        Ok(_) => Err("Something went wrong, please contact the author".to_string()),
        Err(e) => Err(e),
    }
}
